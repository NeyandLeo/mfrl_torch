...
class MFQNet(nn.Module):
    def __init__(self,input_channels=5,num_actions=21, hidden_size=256):
        ...
        # 3. mean_action (shape = (B,21)) 经过两层全连接
        self.fc_mean1 = nn.Linear(num_actions, 64)
        self.fc_mean2 = nn.Linear(64, 64)

        # 拼接后 y 的维度 = 192 + 64 = 256

        # 4. y 经过三层全连接输出 Q 值
        # 这里可以根据需要调整隐藏层的大小，这里示例设为 hidden_size
        self.fc1 = nn.Linear(192 + 64, hidden_size)
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        self.fc3 = nn.Linear(hidden_size, num_actions)

    def forward(self, obs, feature, mean_action):
        """
        obs: (B, 13, 13, 5)
        feature: (B, 1)
        mean_action: (B, 21)
        ...
        # mean_action 两层全连接
        x_mean = F.relu(self.fc_mean1(mean_action))
        x_mean = F.relu(self.fc_mean2(x_mean))

        # 连接 x_cat 和 x_mean => y
        y = torch.cat([x_cat, x_mean], dim=1)  # (B, 192 + 64 = 256)
...
class MFQModel(nn.Module):
    ...
    def train(self, batch):
        ...
        # mean_action: (B, 21) 或按需求处理
        mean_action = torch.tensor(mean_action, dtype=torch.float32, device=self.device)  # (B, 21)
        ...
        q_values_all = self.qnet(old_obs, idx, mean_action)  # (B, num_actions)
        ...
        with torch.no_grad():
            ...
            next_q_values_all = self.target_qnet(obs, idx, mean_action)  # (B, num_actions)
    ...
    def get_action(self, obs, idx, mean_action):
    ...
    def save_model(self,model_name="mfq"):
